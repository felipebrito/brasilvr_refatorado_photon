    ...
    private void SwitchStereoRenderer()
    {
      switch (renderMode)
      {
        case RenderMode.NORMAL:
          if (quadRenderer != null) quadRenderer.SetActive(true);
          if (sphereRenderer != null) sphereRenderer.SetActive(false);
          if (cubeRenderer != null) cubeRenderer.SetActive(false);
          if (eacRenderer != null) eacRenderer.SetActive(false);
          if (domeRenderer != null) domeRenderer.SetActive(false);
        ...
        case RenderMode._360:
          ...
          if (quadRenderer != null) quadRenderer.SetActive(false);
          if (domeRenderer != null) domeRenderer.SetActive(false);
          ...
          if (projectionType == ProjectionType.CUBEMAP)
          {
            if (sphereRenderer != null) sphereRenderer.SetActive(false);
            if (eacRenderer != null) eacRenderer.SetActive(false);
            if (cubeRenderer != null) cubeRenderer.SetActive(true);
          ...
          else if (projectionType == ProjectionType.EQUIRECT)
          {
            if (cubeRenderer != null) cubeRenderer.SetActive(false);
            if (eacRenderer != null) eacRenderer.SetActive(false);
            if (sphereRenderer != null) sphereRenderer.SetActive(true);
          ...
          {
            if (cubeRenderer != null) cubeRenderer.SetActive(false);
            if (eacRenderer != null) eacRenderer.SetActive(true);
            if (sphereRenderer != null) sphereRenderer.SetActive(false);

            EACMapping mapping = eacRenderer != null ? eacRenderer.GetComponent<EACMapping>() : null;
            if (mapping != null)
            {
        ...
        case RenderMode._180:
          ...
          if (sphereRenderer != null) sphereRenderer.SetActive(false);
          if (cubeRenderer != null) cubeRenderer.SetActive(false);
          if (domeRenderer != null) domeRenderer.SetActive(true);
      ...
      {
        Debug.LogErrorFormat(LOG_FORMAT, "targetRenderer is null!");
        return;
      }
      ...
      {
        Debug.LogErrorFormat(LOG_FORMAT, "targetMaterialRenderer is null on " + targetRenderer.name);
        return;
      }

      if (targetMaterialRenderer.material == null)
      {
        Debug.LogErrorFormat(LOG_FORMAT, "targetMaterialRenderer.material is null on " + targetRenderer.name);
        return;
      }
